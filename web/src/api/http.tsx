...
const instance: AxiosInstance = axios.create({
    baseURL: isEnv ? config.baseEnvURL : config.baseURL,
    timeout: 0,
...
instance.interceptors.request.use((config: AxiosRequestConfig) => {
    ...
    if (!((config.params && (config.params as any).showLoading === false) || (config.data && (config.data as any).showLoading === false))) {
...
instance.interceptors.response.use((response: AxiosResponse) => {
    if (!((response.config.params && (response.config.params as any).showLoading === false) || (response.config.data && (response.config.data as any).showLoading === false))) {
        Loading.remove();
    } 
